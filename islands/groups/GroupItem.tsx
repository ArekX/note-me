...
import {
    activeMenuRecordId,
    clearPopupOwner,
} from "$frontend/stores/active-sidebar-item.ts";
...
export type RecordItem = { type: "group"; record: GroupRecord } | {
    type: "note";
    record: NoteRecord;
};
...
export default function GroupItem({
    ...
    const handleOpenFolder = () => {
        ...
        if (
            ...
            (type == "note") ||
            (type == "group" &&
                (!record.has_notes && !record.has_subgroups &&
                    children.length == 0))
        ) {
            return;
        }
        ...
        >
            <div
                ...
                class={`relative group-item hover:bg-gray-600 ${
                    activeMenuRecordId.value == container.record.id
                        ? "opened-menu"
                        : ""
                }`}
            ...
            >
                {!container.edit_mode && !container.is_processing && (
                    <div class="absolute right-0 flex items-center group-item-actions pr-1">
                        ...
                        <IconMenu
                            iconName="dots-horizontal-rounded"
                        ...
                        <span class="group-item-name pl-2 pr-2">
                            <Icon
                                ...
                                type={container.type == "group" &&
                                        (container.record.has_notes ||
                                            container.record.has_subgroups ||
