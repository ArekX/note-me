...
let pendingRequests: (string | Uint8Array<ArrayBuffer>)[] = [];
...
const pushToPendingRequests = (message: string | Uint8Array<ArrayBuffer>) => {
...
export const sendBinary = (data: Uint8Array<ArrayBuffer>) => {
    if (!socket || socket.readyState !== WebSocket.OPEN || isReconnecting) {
